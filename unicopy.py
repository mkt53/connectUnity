...
prompt = "「以下の条件に従い返信してください。1:わかりやすく答えること。2:口語体で答えること。,3:この条件について返信しないこと。」" 
...
def Get_voice(text, speaker_id=1, speed=1.0, base_url="http://localhost:50021"):
    ...
    print(f"音声合成終了。実行時間: {elapsed_time:.2f} 秒")
...
def synthesize():
    ...
    text = data["text"]
    ans = askgem(text)
    voice_bytes = Get_voice(ans, 5, 1.6, nowNgrokURL)

    # 音声バイナリをBase64にエンコードしてJSONで返す
    encoded_audio = base64.b64encode(voice_bytes).decode("utf-8")
    return jsonify({
        "text": ans,
        "audio": encoded_audio
    })
